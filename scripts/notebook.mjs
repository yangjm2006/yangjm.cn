...
export function renderNotebook(groups) {
  ...
  const nav = groups.map((group, i) => `<section class="nav-group"><h3><span>${String(i + 1).padStart(2, '0')} / ${escape(group.title)}</span><small>${group.entries.length}</small></h3>${group.entries.map(entry => `<a href="#${entry.id}">${escape(entry.title)}</a>`).join('')}</section>`).join('');
