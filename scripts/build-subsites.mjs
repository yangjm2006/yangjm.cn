...
const awards = [
  ['2026-04', '2026年山东大学程序设计精英挑战赛', '冠军'],
  ['2026-05', '2026年ICPC山东省赛', '金奖'],
  ['2026-06', '2026年中国大学生程序设计竞赛全国邀请赛（贵阳）', '银奖'],
  ['2026-07', '2026年ICPC沈阳全国邀请赛', '铜奖'],
];
function awardColor(result) {
  if (/(冠军|亚军|季军|第[一二三]名)/.test(result)) return 'red';
  if (/(金奖|一等奖)/.test(result)) return 'gold';
  if (/(银奖|二等奖)/.test(result)) return 'silver';
  if (/(铜奖|三等奖)/.test(result)) return 'bronze';
  return '';
}
const awardHtml = awards.map(([date, contest, result]) => {
  const color = awardColor(result);
  return `<article class="award"><time datetime="${escape(date)}">${escape(date).replace('-', '.')}</time><h3>${escape(contest)}</h3><span class="award-result${color ? ` award-result--${color}` : ''}">${escape(result)}</span></article>`;
}).join('');
if (!process.env.NOTEBOOK_ONLY) await save('public/oi/index.html', shell('算法竞赛', 'oi-page', `<main id="content" class="oi-main"><section class="oi-intro"><div><p class="eyebrow">YANGJM / COMPETITIVE PROGRAMMING</p><h1>思考，编码，<br><em>再进一步。</em></h1><p class="lead">我是 yangjm，山东大学软件工程专业。<br>这里记录我的算法竞赛经历。</p></div><div class="contest-note"><span class="eyebrow">ALGORITHM JOURNAL</span><div class="code-art" aria-hidden="true"><span class="code-keyword">while</span> (<span class="code-condition">curious</span>) {<br>　<span class="code-learn">learn</span>();<br>　<span class="code-solve">solve</span>();<br>　<span class="code-grow">grow</span>();<br>}</div><a href="https://banzi.yangjm.cn">翻开我的算法板子 <span aria-hidden="true">↗</span></a></div></section><section class="oi-section"><div class="section-label"><span>01 / ONLINE JUDGES</span><h2>练习的地方</h2></div><div class="platforms">${platforms.map(([id,name,url]) => `<a class="platform" data-platform="${id}" href="${url}" target="_blank" rel="noopener noreferrer"><span class="platform-mark" aria-label="${name}">${icons[id]}</span><div class="judge-identity"><h3>${name}</h3><p><span class="judge-handle">yangjm</span></p></div><span aria-hidden="true">↗</span></a>`).join('')}</div></section><section class="oi-section"><div class="section-label"><span>02 / TEAM HISTORY</span><h2>一起解题的人</h2></div><div class="team-list"><article class="team"><p>2026.04 — 至今</p><div><h3>Rolling Girl</h3><span class="status">当前队伍</span></div></article><article class="team"><p>2025.08 — 2025.12</p><div><h3>零一之间</h3></div></article></div></section><section class="oi-section"><div class="section-label"><span>03 / ACHIEVEMENTS</span><h2>比赛与奖项</h2></div><div class="award-list">${awardHtml}</div></section></main>`));
